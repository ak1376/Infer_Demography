...
def infer_common_params(
    features_df: pd.DataFrame, targets_df: pd.DataFrame, tools: tuple[str, ...]
) -> list[str]:
    ...
    *,
    ...
    if train_pct + tune_pct >= 1.0:
        raise ValueError(
            ...
            "must be < 1.0 so there is room for validation."
        )
    val_pct = 1.0 - train_pct - tune_pct
    ...
    print(
        f"[INFO] Split fractions: train={train_pct:.3f}, "
        f"tune={tune_pct:.3f}, val={val_pct:.3f}"
    )
    ...
    print(
        f"[INFO] Extra Features: use_fim_features={use_fim_features}, use_residuals={use_resid}, residual_engines={resid_engs}"
    )
    ...
    print(
        f"[INFO] Outlier filtering (extreme only): kept {kept_count} / {total_rows} rows "
    ...
    if outlier_records:
        ...
        to_show = (
            len(outliers_df) if show_all else min(int(preview_rows), len(outliers_df))
        )
    ...
    assert set(train_idx).isdisjoint(tune_idx)
