...
def _parse_args():
    ...
    p.add_argument(
        "--config", type=Path, required=True, help="JSON experiment configuration file"
    )
    p.add_argument(
        "--model-py",
        type=str,
...
def create_start_dict_with_fixed(
    config: Dict, fixed_params: Dict[str, float]
) -> Dict[str, float]:
...
def classify_parameter_type(param_name: str) -> str:
    ...
    if any(
        x in param_lower
        for x in ["n0", "n1", "n2" "size", "anc", "afr", "eur", "bottleneck", "recover"]
    ):
        return "population_size"
    elif any(
        x in param_lower for x in ["t", "time", "split", "expansion", "divergence"]
    ):
        return "time"
        ...
        try:
            if mode == "dadi":
                ...
                if hasattr(sfs, "pop_ids") and sfs.pop_ids:
                    ...
                    sample_sizes = {
                        pop_id: (sfs.shape[i] - 1) // 2
                        for i, pop_id in enumerate(sfs.pop_ids)
                    }
                ...
                if hasattr(sfs, "pop_ids") and sfs.pop_ids:
                    ...
                    sample_sizes = OrderedDict(
                        (f"pop{i}", (n - 1) // 2) for i, n in enumerate(sfs.shape)
                    )
...
def generate_likelihood_profiles(
    ...
    for i, param_name in enumerate(param_names):
        ...
        true_value = (
            ground_truth_params.get(param_name) if ground_truth_params else None
        )
...
def run_inference_mode(
    ...
    result = {
        "mode": mode,
        "best_params": (
            {k: float(v) for k, v in zip(start_dict.keys(), best_params)}
    ...
    if best_params is not None:
        try:
            ...
            sim_params_file = (
                outdir.parent / "sim_params.pkl" if outdir.parent.exists() else None
            )
...
def main():
    ...
    if args.mode == "both":
        for mode in ["dadi", "moments"]:
            run_inference_mode(
                mode, sfs, config, demo_func, start_dict, args.outdir, fixed_params
            )
    else:
        run_inference_mode(
            args.mode, sfs, config, demo_func, start_dict, args.outdir, fixed_params
        )
