...
def run_simulation(simulation_dir: Path, experiment_config: Path, model_type: str,
    ...
    if not sel_cfg.get("enabled", False):
        raise ValueError("This runner is BGS-only. Set selection.enabled = true in your config.")
...
def main():
    ...
    cli.add_argument("--model-type", required=True,
                     choices=["bottleneck", "split_isolation", "split_migration", "drosophila_three_epoch"],  # ← added drosophila
