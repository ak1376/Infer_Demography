...
USE_BOBYQA = (
    False  # False → LD_LBFGS (finite-diff grad). True → LN_BOBYQA (derivative-free)
)
VERBOSE_SFS = True
RTOL_SFS = 1e-8
MAXEVAL_SFS = 800
...
def _aggregate_and_optimise_momentsld(cfg: Path, exp_root: Path, rep: int, r_bins: str):
    ...
    pm = {k: (lo + hi) / 2 for k, (lo, hi) in priors.items()}
    p0 = [
        pm["N1"] / pm["N0"],
        pm["N2"] / pm["N0"],
        2 * pm["m12"] * pm["N0"],
...
def _aggregate_and_optimise_sfs(cfg: Path, exp_root: Path, rep: int, backend: str):
    ...
    root = (
        ...
        / "inferences"
        / f"sim_{rep}"
        / ("Moments" if backend == "moments" else "Dadi")
    )
    ...
    out = {
        "best_params": dict(
            zip(["N_ANC", "N1", "N2", "T", "m12", "m21"], map(float, fitted_pars))
        ),
        "best_ll": best_ll,
        "status": status,
    }
    ...
    logging.info(
        "rep %d: %s SFS optimisation done (LL=%.3f, status=%d)",
        rep,
        backend,
        best_ll,
        status,
...
def _parse_args():
    ...
    p.add_argument(
        "--mode",
        choices=["momentsld", "moments", "dadi"],
        default="momentsld",
        help="Which optimisation path to run",
    )
