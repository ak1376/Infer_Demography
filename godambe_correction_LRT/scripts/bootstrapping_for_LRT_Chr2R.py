# bootstrapping_for_LRT_Chr2R.py
#
# Single-arm variant of bootstrapping_for_LRT.py: same Godambe-adjusted LRT for
# CO growth, but run on Chr2R alone instead of the pooled Chr2L+Chr2R SFS, using
# the per-arm fits from godambe_correction_LRT/real_arms/Chr2R/sfs_fit_{simple,
...
----------------------------------
The demographic fits here use Chr2R's OWN SFS (not pooled with any other arm),
so the bootstrap resamples blocks tiling Chr2R only. Chr2R is tiled into
...
# data_sfs is Chr2R's own SFS -- the fits below were run on this, not a
# multi-arm combined spectrum.
combined_sfs_path = f"{DROSO_DIR}/Chr2R/unfolded.sfs.pkl"
simple_fit_path = f"{GODAMBE_ARM_DIR}/Chr2R/sfs_fit_simple/best_fit.pkl"
complex_fit_path = f"{GODAMBE_ARM_DIR}/Chr2R/sfs_fit_complex/best_fit.pkl"
out_dir = "/sietch_colab/akapoor/Infer_Demography/figures/godambe_Chr2R"
...
ax1.set_title("Block-size sensitivity of Godambe-adjusted LRT (Chr2R)")
...
ax.set_title("Score-variance estimate J across block sizes (Chr2R)")
...
ax.set_title("Godambe adjustment factor across block sizes (Chr2R)")
ax.set_xticks(block_kb_arr)
ax.legend(fontsize=8)
fig.tight_layout()
