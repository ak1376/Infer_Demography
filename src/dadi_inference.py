...
def diffusion_sfs_dadi(
    ...
    theta = 4.0 * float(p_dict[param_names[0]]) * float(mutation_rate) * float(sequence_length)
    fs *= theta
    return fs
...
def fit_model(
    ...
    fix_vals  = np.array([max(float(fixed_params[param_names[i]]), 1e-300) for i in fix_idx], dtype=float)

    # Bound check for fixeds
    for i, v in zip(fix_idx, fix_vals):
        if not (lower_b[i] <= v <= upper_b[i]):
            raise ValueError(f"Fixed value {param_names[i]}={v} outside bounds [{lower_b[i]}, {upper_b[i]}].")
    ...
    def objective_log10(log10_free_vec, grad):
        ...
        try:
            ...
            print(f"[LL={ll:.6g}] log10_free={np.array2string(np.asarray(log10_free_vec), precision=4)}")
            return ll
    ...
    # ── optimise ────────────────────────────────────────────────────────
    print("▶ dadi NLopt optimisation –", datetime.datetime.now().isoformat(timespec='seconds'))
