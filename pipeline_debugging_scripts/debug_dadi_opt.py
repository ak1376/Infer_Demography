...
msprime/stdpopsim simulation + dadi expected SFS + nlopt(L-BFGS) optimization +
...
def save_demes_graph(g: demes.Graph, prefix: str, out_dir: str | Path = ".") -> Tuple[Path, Path]:
    ...
    try:
        ...
        # keep going; PNG is usually what you care about
        pass
...
def msprime_model(
    ...
    *,
    ...
    graph_prefix: str = "IM_symmetric_AB",
...
# =============================================================================
# dadi expected SFS + likelihood (kept structurally identical to moments version)
# =============================================================================

def expected_sfs(log10_params: np.ndarray, sample_size: Sequence[int], muL: float) -> dadi.Spectrum:
    """
    log10_params corresponds to:
      [N_anc, N_A, N_B, T, m] in log10 space.

    muL is mu * L (so theta = 4 * N_anc * muL).
    Returns a dadi.Spectrum of expected *counts* (theta-scaled),
    analogous to moments.Spectrum.from_demes(..., theta=...).
    """
    N_anc, N_A, N_B, T, m = 10 ** np.asarray(log10_params, dtype=float)

    # dadi expects haploid ns = (nA, nB)
    ns = tuple(int(n) for n in sample_size)
    sampled_demes = ["A", "B"]

    # mimic your old dynamic pts behavior
    pts_base = [10, 20, 30]
    n_max = max(ns)
    pts_l = [n_max + pts_base[0], n_max + pts_base[1], n_max + pts_base[2]]

    def raw_wrapper(params_vec, ns_local, pts):
        N_anc2, N_A2, N_B2, T2, m2 = params_vec
        g = build_demes_graph(N_anc2, N_A2, N_B2, T2, m2)
        ...
        theta = 4.0 * float(N_anc2) * float(muL)
        return model * theta

    func_ex = dadi.Numerics.make_extrap_func(raw_wrapper)

    params_vec = np.array([N_anc, N_A, N_B, T, m], dtype=float)
    model_counts = func_ex(params_vec, ns, pts_l)

    return dadi.Spectrum(np.maximum(model_counts, 1e-300))
...
def optimize_lbfgs(
    ...
    observed_sfs: dadi.Spectrum,
    muL: float,
    verbose: bool = False,
    rtol: float = 1e-8,
):
    """
    Maximize Poisson composite log-likelihood in log10 space.

    IMPORTANT: nlopt.LD_LBFGS can terminate with a failure code (e.g. roundoff limited)
    and the Python wrapper raises nlopt.runtime_error. We track best-so-far and return it.
    """
    ...
    def loglikelihood(log10_params: np.ndarray) -> float:
        exp_sfs = expected_sfs(log10_params, sample_size, muL)
    ...
    # Track best point we ever evaluated (so we can recover if NLopt errors)
    best = {"ll": -np.inf, "x": None}
    ...
    def objective(log10_params: np.ndarray, grad: np.ndarray) -> float:
        try:
            ...
            # update best-so-far
            if np.isfinite(ll) and ll > best["ll"]:
                best["ll"] = ll
                best["x"] = np.array(log10_params, dtype=float, copy=True)
            ...
            if verbose:
                print(f"[obj failed] {e} at {log10_params}")
            if grad.size > 0:
                grad[:] = 0.0
            return BAD

    opt = nlopt.opt(nlopt.LD_LBFGS, start_values.size)
    opt.set_lower_bounds(np.log10(np.maximum(lower_bounds, 1e-300)))
    opt.set_upper_bounds(np.log10(np.maximum(upper_bounds, 1e-300)))
    opt.set_max_objective(objective)
    opt.set_ftol_rel(rtol)
    opt.set_maxeval(500)
    opt.set_xtol_rel(1e-6)

    x0 = np.log10(np.maximum(start_values, 1e-300))

    try:
        fitted_log10 = opt.optimize(x0)
        code = opt.last_optimize_result()
    except Exception as e:
        # NLopt failed (often roundoff limited). Return best-so-far.
        code = opt.last_optimize_result()
        if best["x"] is None:
            fitted_log10 = x0
            if verbose:
                print(f"[nlopt failed: {e}] no best point recorded; returning start.")
        else:
            fitted_log10 = best["x"]
            if verbose:
                print(f"[nlopt failed: {e}] returning best-so-far ll={best['ll']:.6f}.")

    fitted = 10 ** fitted_log10
    fitted_sfs = expected_sfs(fitted_log10, sample_size, muL)
...
def sanity_check(num_reps: int = 100, out_dir: str | Path = ".") -> None:
    ...
    Compare:
      ...
      - dadi expected SFS from demes graph (theta-scaled counts)
    ...
    for ts in tsg:
        tmp = ts.allele_frequency_spectrum(
            ...
            polarised=True,
        ) * mu  # convert branch length to expected mutation counts
        mc_sfs = tmp if mc_sfs is None else (mc_sfs + tmp)

    mc_sfs /= num_reps

    exp_sfs = expected_sfs(np.log10(params), [n - 1 for n in mc_sfs.shape], muL)
    ...
    plt.xlabel("dadi expected SFS (counts)")
...
if __name__ == "__main__":
    ...
    # Build demes graph (NOT msprime.Demography) for stdpopsim
    g = build_demes_graph(*true_pars)
    model = define_sps_model(g)

    contig = _contig_from_cfg()
    samples = {"A": 5, "B": 5}
    seed = 1
