...
2) Backends:
   - msprime
   - stdpopsim (requires src.stdpopsim_wrappers.define_sps_model(g))

3) Optional 1D likelihood scan over m (log-spaced grid) for a subset of sims:
   - saves m_grid.npy, ll_grid.npy, ll_vs_m.png in each sim directory

4) NEW: Expected SFS equivalence check between demes formulations (per sim + aggregate):
   For each sim (same theta, same parameters, same m_true):
     ...
     - expected_sfs_relerr.png (heatmap of |relerr|)
     - expected_sfs_metrics.json
   Across sims:
     - expected_sfs_metrics.csv

Outputs:
- per-sim directories with demes graph + true/fitted m (+ optional ll scan)
- plots: true_m vs fitted_m (and formulation vs formulation if both)
- expected SFS comparison artifacts under out_root/expected_sfs_compare/
"""
...
def build_no_anc(N_anc, N_YRI, N_CEU, T_split, m) -> Tuple[demes.Graph, msprime.Demography]:
    """
    NO_ANC formulation:
      - YRI exists from present back to infinity,
        with an ancestral epoch [INF, T_split] of size N_anc
        and a post-split epoch [T_split, 0] of size N_YRI
      - CEU branches off at T_split
    """
    b = demes.Builder(time_units="generations", generation_time=1)
    ...
    *,
    ...
    if sim_backend == "stdpopsim":
        ...
        if experiment_config is not None:
            # optional: allow your pipeline config to define contig/DFE
            try:
...
# =============================================================================
# Expected SFS comparison between formulations
# =============================================================================

def _safe_relerr(diff: np.ndarray, base: np.ndarray, eps: float = 1e-300) -> np.ndarray:
    return diff / np.maximum(base, eps)
...
def expected_sfs_from_graph(
    ...
    *,
    ...
    # sample_sizes are haploid sample counts (msprime "samples" are haploids)
    sample_sizes = [int(nYRI), int(nCEU)]
...
def save_expected_sfs_comparison(
    *,
    out_dir: Path,
    g_with: demes.Graph,
    g_no: demes.Graph,
    ...
    sfs_with = expected_sfs_from_graph(
        g_with, nYRI=nYRI, nCEU=nCEU, N_anc=N_anc, mut_rate_times_L=mut_rate_times_L
    )
    sfs_no = expected_sfs_from_graph(
        g_no, nYRI=nYRI, nCEU=nCEU, N_anc=N_anc, mut_rate_times_L=mut_rate_times_L
    )

    A = np.asarray(sfs_with, dtype=float)
    B = np.asarray(sfs_no, dtype=float)

    diff = A - B
    relerr = _safe_relerr(diff, A)  # relative to WITH_ANC baseline

    np.save(out_dir / "expected_sfs_with_anc.npy", A)
    np.save(out_dir / "expected_sfs_no_anc.npy", B)
    ...
    rel_l1 = float(np.sum(np.abs(diff)) / np.maximum(np.sum(np.abs(A)), 1e-300))
    max_abs_rel = float(np.max(np.abs(relerr)))

    metrics = {
        "l1": l1,
        "l2": l2,
        "rel_l1_vs_with": rel_l1,
        "max_abs_rel_vs_with": max_abs_rel,
    }

    (out_dir / "expected_sfs_metrics.json").write_text(
        json.dumps(metrics, indent=2), encoding="utf-8"
    )

    # scatter plot: WITH vs NO
    fig, ax = plt.subplots(figsize=(5, 5))
    ax.plot(A.ravel(), B.ravel(), "o", markersize=2, alpha=0.7)
    lo = float(min(A.min(), B.min()))
    hi = float(max(A.max(), B.max()))
    ax.plot([lo, hi], [lo, hi], "--")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel("Expected SFS bin (WITH_ANC)")
    ax.set_ylabel("Expected SFS bin (NO_ANC)")
    ax.set_title(f"{title_prefix} expected SFS: WITH vs NO")
    fig.tight_layout()
    fig.savefig(out_dir / "expected_sfs_compare.png", dpi=200)
    plt.close(fig)

    # heatmap of absolute relative error
    fig, ax = plt.subplots(figsize=(6, 4))
    im = ax.imshow(np.abs(relerr), aspect="auto", origin="lower")
    ax.set_title(f"{title_prefix} |abs(relerr)| vs WITH_ANC")
    ax.set_xlabel("CEU freq bin")
    ax.set_ylabel("YRI freq bin")
...
def write_expected_sfs_metrics_csv(rows: List[Dict[str, float]], out_csv: Path) -> None:
    if not rows:
        return
...
def parse_args():
    ...
    # Separate bounds for TRUE m sampling (so you can force high-m truths)
    p.add_argument(
        "--m-true-lb",
        type=float,
        default=None,
        help="Lower bound for sampling ground-truth m (default: use --m-lb).",
    )
    p.add_argument(
        "--m-true-ub",
        type=float,
        default=None,
        help="Upper bound for sampling ground-truth m (default: use --m-ub).",
    )

    # Likelihood scan controls
    ...
    # Expected SFS compare (only applies if formulations==both)
    p.add_argument(
        "--expected-sfs-compare",
        action="store_true",
        help="Compute expected SFS for WITH_ANC and NO_ANC and compare per sim + aggregate (requires --formulations both).",
    )
...
def main():
    ...
    # true-m sampling bounds
    m_true_lb = float(args.m_true_lb) if args.m_true_lb is not None else m_lb
    m_true_ub = float(args.m_true_ub) if args.m_true_ub is not None else m_ub
    ...
    for i in range(args.num_sims):
        m_true = sample_loguniform(rng, m_true_lb, m_true_ub)
        true_m[i] = m_true

        # Optional expected-SFS equivalence check (only meaningful when both formulations are present)
        if args.expected_sfs_compare and ("with_anc" in run_formulations) and ("no_anc" in run_formulations):
            g_with, _ = build_with_anc(N_anc, N_YRI, N_CEU, T_split, m_true)
            g_no, _ = build_no_anc(N_anc, N_YRI, N_CEU, T_split, m_true)
            exp_dir = ensure_dir(out_root / "expected_sfs_compare" / f"sim_{i:04d}")
            metrics = save_expected_sfs_comparison(
                out_dir=exp_dir,
                g_with=g_with,
                g_no=g_no,
                nYRI=nYRI,
                ...
                N_anc=N_anc,
                mut_rate_times_L=mut_rate_times_L,
                title_prefix=f"sim_{i:04d} (m={m_true:.3e})",
            )
            expected_rows.append({"sim": float(i), "m_true": float(m_true), **metrics})
        ...
        for f in run_formulations:
            ...
            # likelihood scan (optional; only for first K sims)
    ...
    # aggregate expected SFS discrepancy summary
    if expected_rows:
        write_expected_sfs_metrics_csv(expected_rows, out_root / "expected_sfs_metrics.csv")
    ...
    print(f"Plots:        {plots}")
    if expected_rows:
        print(f"Expected-SFS: {out_root / 'expected_sfs_metrics.csv'}")
