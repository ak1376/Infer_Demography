...
sim_id=$(submit_array "$(array_spec "$NUM_DRAWS" "$BATCH_SIZE")" \
  bash_scripts/simulation/running_simulation.sh); [[ -n "$sim_id" ]]
...
mat_id=$(submit_array "$(array_spec "$NUM_DRAWS" "$BATCH_SIZE")" \
  $(dep_afterany "$sim_id") bash_scripts/simulation/materialize_windows.sh); [[ -n "$mat_id" ]]
...
win_id=$(submit_array "$(array_spec "$(( NUM_DRAWS * NUM_WINDOWS ))" "$BATCH_SIZE" "$MAX_CONCURRENT_WINDOWS")" \
  $(dep_afterany "$mat_id") bash_scripts/simulation/build_windows.sh); [[ -n "$win_id" ]]
...
ld_id=$(submit_array "$(array_spec "$(( NUM_DRAWS * NUM_WINDOWS ))" "$BATCH_SIZE")" \
  $(dep_afterany "$win_id") "${LD_STATS_SBATCH_OPTS[@]}" bash_scripts/simulation/LD_stats_windows.sh); [[ -n "$ld_id" ]]
...
momLD_prep_id=$(submit_array "$(array_spec "$NUM_DRAWS" "$BATCH_SIZE")" \
  $(dep_afterany "$ld_id") bash_scripts/simulation/MomentsLD_prep.sh); [[ -n "$momLD_prep_id" ]]

export BATCH_SIZE=50
momLD_id=$(submit_array "$(array_spec "$(( NUM_DRAWS * NUM_OPTIMS ))" "$BATCH_SIZE" 5000)" \
  $(dep_afterany "$momLD_prep_id") bash_scripts/simulation/MomentsLD.sh); [[ -n "$momLD_id" ]]

export BATCH_SIZE=1
momLD_agg_id=$(submit_array "$(array_spec "$NUM_DRAWS" "$BATCH_SIZE")" \
  $(dep_afterany "$momLD_id") bash_scripts/simulation/aggregate_momentsld.sh); [[ -n "$momLD_agg_id" ]]
...
if [[ "$RUN_MOMENTS_DADI_MODE" == "parallel" ]]; then
  mom_id=$(submit_array "$(array_spec "$(( NUM_DRAWS * NUM_OPTIMS ))" "$BATCH_SIZE" 100)" \
    $(dep_afterany "$ld_id") bash_scripts/simulation/moments.sh); [[ -n "$mom_id" ]]
  dadi_id=$(submit_array "$(array_spec "$(( NUM_DRAWS * NUM_OPTIMS ))" "$BATCH_SIZE")" \
    $(dep_afterany "$ld_id") bash_scripts/simulation/dadi.sh); [[ -n "$dadi_id" ]]
else
  mom_id=$(submit_array "$(array_spec "$(( NUM_DRAWS * NUM_OPTIMS ))" "$BATCH_SIZE" 100)" \
    $(dep_afterany "$ld_id") bash_scripts/simulation/moments.sh); [[ -n "$mom_id" ]]
  dadi_id=$(submit_array "$(array_spec "$(( NUM_DRAWS * NUM_OPTIMS ))" "$BATCH_SIZE")" \
    $(dep_afterany "$mom_id") bash_scripts/simulation/dadi.sh); [[ -n "$dadi_id" ]]
fi
...
agg_id=$(submit_array "$(array_spec "$NUM_DRAWS" "$BATCH_SIZE")" \
  --dependency=afterany:$mom_id:$dadi_id:$momLD_agg_id bash_scripts/simulation/aggregate_moments_dadi.sh); [[ -n "$agg_id" ]]
...
fim_id=$(submit_array "$(array_spec "$NUM_DRAWS" "$BATCH_SIZE")" \
  $(dep_afterany "$agg_id") bash_scripts/simulation/compute_fim.sh); [[ -n "$fim_id" ]]
resid_id=$(submit_array "$(array_spec "$NUM_DRAWS" "$BATCH_SIZE")" \
  $(dep_afterany "$agg_id") bash_scripts/simulation/compute_residuals.sh); [[ -n "$resid_id" ]]

# --- 9. combine_results ---
export BATCH_SIZE=1
comb_id=$(submit_array "$(array_spec "$NUM_DRAWS" "$BATCH_SIZE")" \
  --dependency=afterany:$momLD_agg_id:$agg_id:$fim_id:$resid_id bash_scripts/modeling/run_combine.sh); [[ -n "$comb_id" ]]

# --- 10. build modeling dataset (single job, no array) ---
feat_id=$(submit --dependency=afterany:$comb_id bash_scripts/modeling/aggregate_features.sh); [[ -n "$feat_id" ]]
