#!/bin/bash
#SBATCH --job-name=fim
#SBATCH --output=logs/fim_%A_%a.out
#SBATCH --error=logs/fim_%A_%a.err
...
# FIM is always computed for whichever engines the config lists (default:
# moments) -- whether it's later used as a model feature is a separate
# decision made downstream in feature_extraction.py via --use-fim-features.
mapfile -t ENGINES < <(jq -r '(.fim_engines // ["moments"]) | if type=="array" then .[] else . end' "$CFG")
...
for sid in $(seq "$RUN_START" "$RUN_END"); do
  ...
  for eng in "${ENGINES[@]}"; do
    ...
    if [[ -f "$fit" ]]; then
      TARGETS+=("experiments/${MODEL}/inferences/sim_${sid}/fim/${eng}.fim.npy")
    else
